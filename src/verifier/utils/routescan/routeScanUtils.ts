...
export async function handleBalanceCheck(config: BalanceCheckCredConfig, check_address: Address): Promise<CredResult> {
  if (config.network !== ETHEREUM_MAINNET_ID && config.network !== BERACHAIN_TESTNET_ID) {
    throw new Error(
      `Unsupported network: ${config.network}. This function only supports Ethereum mainnet and BeraChain testnet.`,
    );
  }
